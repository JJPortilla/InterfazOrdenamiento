...
public class interfazOrdenamiento extends JFrame {
	...
	public interfazOrdenamiento() {
		...
		textArregloOrdenado.setBounds(63, 200, 274, 97);
		contentPane.add(textArregloOrdenado);

		JButton btnOrdenBurbuja = new JButton("Burbuja");
		btnOrdenBurbuja.setBounds(359, 202, 85, 21);
		...
		btnSeleccion.setBounds(359, 233, 85, 21);
		...
		btnInsercion.setBounds(359, 264, 85, 21);
		...
		btnBurbujaMejorado.setBounds(359, 295, 85, 21);
		...
		JButton btnIngresarTamano = new JButton("Ingresar Tama�o");
		...
		btnLimpiar.setBounds(370, 328, 150, 21);
	...
	private void ingresarTamano() {
		try {
			...
			JOptionPane.showMessageDialog(this, "Tama�o ingresado correctamente");
		} catch (NumberFormatException e) {
			JOptionPane.showMessageDialog(this, "Ingrese un tama�o v�lido (n�mero entero positivo)");
		}
	}
	...
	private void ingresarDatos() {
		try {
			...
			if (valoresStr.length != tamano) {
				throw new Exception("N�mero de valores no coincide con el tama�o del arreglo");
			}
	...
	private void ordenarBurbuja() {
		try {
			...
			String resultado = sorter.sortByBubbleAscendInterfaz(valores, true);
	...
	private void ordenarSeleccion() {
		try {
			...
			String resultado = sorter.sortBySelectionAscendInterfaz(valores, true);
			textArregloOrdenado.setText(resultado);
		} catch (Exception e) {
			textArregloOrdenado.setText("Error en los datos ingresados");
		}
	...
	private void ordenarInsercion() {
		try {
			...
			String resultado = sorter.sortByInsertionAscendInterfaz(valores, true);
			textArregloOrdenado.setText(resultado);
		} catch (Exception e) {
			textArregloOrdenado.setText("Error en los datos ingresados");
		}
	...
	private void ordenarBurbujaMejorado() {
		try {
			...
			String resultado = sorter.sortByBubbleUpgradeAscendInterfaz(valores, true);
			textArregloOrdenado.setText(resultado);
		} catch (Exception e) {
			textArregloOrdenado.setText("Error en los datos ingresados");
		}
